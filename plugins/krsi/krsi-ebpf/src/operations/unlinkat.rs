...
fn try_do_unlinkat_x(ctx: FExitContext) -> Result<u32, i64> {
    ...
    const EVT_TYPE: EventType = EventType::Unlinkat;
    let mut writer = auxmap.writer(EVT_TYPE, get_event_num_params(EVT_TYPE))?;

    // Parameter 1: dirfd.
    let dirfd: i32 = unsafe { ctx.arg(0) };
    writer.push(scap::encode_dirfd(dirfd) as i64);

    // Parameter 2: path.
    let path: Filename = wrap_arg(unsafe { ctx.arg(1) });
    writer.push_filename(&path, defs::MAX_PATH, true);

    // Parameter 3: flags.
    match op_data.flags {
        Some(flags) => writer.push(scap::encode_unlinkat_flags(flags)),
        None => writer.push_empty(),
    }
